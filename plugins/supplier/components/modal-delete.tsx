...
export const ModalDelete = () => {
  ...
  const supplierActive = useRecoilValue(supplierActiveState)

  const handleDelete = () => {
    deleteSupplierApi(supplierActive && supplierActive?.id ? supplierActive?.id : 0)
    ...
    <>
      ...
      >
        <Box textAlign={'center'}>
          ...
          <Typography>
            {t('supplierMessageDeleteDescription')} <strong>"{supplierActive?.name}"</strong>
          </Typography>

          <Stack direction={'row'} spacing={2} justifyContent="center" pt={4}>
            <ButtonBase
              btnText={t('supplierBtnClose')}
              handleClick={() => {
                setOpenModalDelete(!openModalDelete)
              }}
            />
            <ButtonDelete btnText={t('supplierBtnDelete')} handleClick={handleDelete} />
          </Stack>
        </Box>
      </MainModal>
    </>
