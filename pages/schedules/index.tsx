...
import { SchedulesContainer } from '@/plugins/schedules/components'
...
export default function SchedulesPage() {
  ...
  useEffect(() => {
    setRecoil(breadcrumbsState, [
      { name: t('dashboard'), to: '/' },
      { name: t('schedulesBreadcrumbsTitle'), to: '' },
    ])
  ...
  useEffect(() => {
    if (accessToken) {
      if (!isUserLoading) {
        if (permissionsRule?.includes('schedules_view')) {
          setLoading(false)
          return
        } else {
          router.push('/404')
  ...
  return (
    <>
      <HeadPage title={t('schedulesHeadTitle') ?? ''} />
      {loading ? <Loading open={true} /> : <SchedulesContainer />}
    </>
  )
}
