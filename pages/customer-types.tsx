...
import { CustomerTypesContainer } from '@/plugins/customer-type/components'
...
export default function CustomerTypesPage() {
  ...
  useEffect(() => {
    setRecoil(breadcrumbsState, [
      { name: t('dashboard'), to: '/' },
      { name: t('customerTypesBreadcrumbsTitle'), to: '' },
    ])
  ...
  useEffect(() => {
    if (accessToken) {
      if (!isUserLoading) {
        if (permissionsRule?.includes('customer_types_view')) {
          setLoading(false)
          return
        } else {
          router.push('/404')
  ...
  return (
    <>
      <HeadPage title={t('customerTypesHeadTitle') ?? ''} />
      {loading ? <Loading open={true} /> : <CustomerTypesContainer />}
    </>
  )
}
