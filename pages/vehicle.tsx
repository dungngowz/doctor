...
import { VehicleContainer } from '@/plugins/vehicle/components'
...
export default function VehiclePage() {
  ...
  useEffect(() => {
    setRecoil(breadcrumbsState, [
      { name: t('dashboard'), to: '/' },
      { name: t('vehicleBreadcrumbsTitle'), to: '' },
    ])
  ...
  useEffect(() => {
    if (accessToken) {
      if (!isUserLoading) {
        if (permissionsRule?.includes('vehicles_view')) {
          setLoading(false)
          return
        } else {
          router.push('/404')
  ...
  return (
    <>
      <HeadPage title={t('vehicleHeadTitle') ?? ''} />
      {loading ? <Loading open={true} /> : <VehicleContainer />}
    </>
  )
}
