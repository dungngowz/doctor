...
import { ProvinceContainer } from '@/plugins/province/components'
...
export default function ProvincePage() {
  ...
  useEffect(() => {
    if (accessToken) {
      if (!isUserLoading) {
        if (permissionsRule?.includes('provinces_view')) {
          setLoading(false)
          return
        } else {
          router.push('/404')
  ...
  return (
    <>
      <HeadPage title={t('provinceTitle') ?? ''} />
      {loading ? <Loading open={true} /> : <ProvinceContainer />}
    </>
  )
}
