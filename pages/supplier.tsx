...
import { SupplierContainer } from '@/plugins/supplier/components'
...
export default function SupplierPage() {
  ...
  useEffect(() => {
    setRecoil(breadcrumbsState, [
      { name: t('dashboard'), to: '/' },
      { name: t('supplierBreadcrumbsTitle'), to: '' },
    ])
  ...
  useEffect(() => {
    if (accessToken) {
      if (!isUserLoading) {
        if (permissionsRule?.includes('suppliers_view')) {
          setLoading(false)
          return
        } else {
          router.push('/404')
  ...
  return (
    <>
      <HeadPage title={t('supplierHeadTitle') ?? ''} />
      {loading ? <Loading open={true} /> : <SupplierContainer />}
    </>
  )
}
