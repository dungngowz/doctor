...
import { StationContainer } from '@/plugins/station/components'
...
export default function StationPage() {
  ...
  useEffect(() => {
    setRecoil(breadcrumbsState, [
      { name: t('dashboard'), to: '/' },
      { name: t('stationBreadcrumbsTitle'), to: '' },
    ])
  ...
  useEffect(() => {
    if (accessToken) {
      if (!isUserLoading) {
        if (permissionsRule?.includes('stations_view')) {
          setLoading(false)
          return
        } else {
          router.push('/404')
  ...
  return (
    <>
      <HeadPage title={t('stationHeadTitle') ?? ''} />
      {loading ? <Loading open={true} /> : <StationContainer />}
    </>
  )
}
