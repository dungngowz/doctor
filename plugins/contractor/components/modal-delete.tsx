...
import { deleteContractorApi } from '../api'
import { contractorActiveState } from '../store'

export const ModalDelete = () => {
  const [openModalDelete, setOpenModalDelete] = useRecoilState(openModalDeleteState)
  const contractorActive = useRecoilValue(contractorActiveState)

  const handleDeleteRegion = () => {
    deleteContractorApi(contractorActive && contractorActive?.id ? contractorActive?.id : 0)
    ...
    <>
      ...
      >
        <Box textAlign={'center'}>
          ...
          <Typography>
            {t('contractorMessageDeleteDescription')} <strong>"{contractorActive?.name}"</strong>
          </Typography>

          <Stack direction={'row'} spacing={2} justifyContent="center" pt={2}>
            <ButtonBase
              btnText={t('contractorBtnClose')}
              handleClick={() => {
                setOpenModalDelete(!openModalDelete)
              }}
            />
            <ButtonDelete btnText={t('contractorBtnDelete')} handleClick={handleDeleteRegion} />
          </Stack>
        </Box>
      </MainModal>
    </>
