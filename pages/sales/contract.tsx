...
import { ContractContainer } from '@/plugins/contract/components'
...
export default function QuotePage() {
  ...
  useEffect(() => {
    setRecoil(breadcrumbsState, [
      { name: t('dashboard'), to: '/' },
      { name: t('contractBreadcrumbsTitle'), to: '' },
    ])
  ...
  useEffect(() => {
    if (accessToken) {
      if (!isUserLoading) {
        if (permissionsRule?.includes('contracts_view')) {
          setLoading(false)
          return
        } else {
          router.push('/404')
  ...
  return (
    <>
      <HeadPage title={t('contractHeadTitle') ?? ''} />
      {loading ? <Loading open={true} /> : <ContractContainer />}
    </>
  )
}
