...
import { StaffContainer } from '@/plugins/staff/components'
...
export default function StaffPage() {
  ...
  useEffect(() => {
    setRecoil(breadcrumbsState, [
      { name: t('dashboard'), to: '/' },
      { name: t('staffBreadcrumbsTitle'), to: '' },
    ])
  ...
  useEffect(() => {
    if (accessToken) {
      if (!isUserLoading) {
        if (permissionsRule?.includes('staffs_view')) {
          setLoading(false)
          return
        } else {
          router.push('/404')
  ...
  return (
    <>
      <HeadPage title={t('staffHeadTitle') ?? ''} />
      {loading ? <Loading open={true} /> : <StaffContainer />}
    </>
  )
}
