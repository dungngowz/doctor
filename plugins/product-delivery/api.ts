...
import { productDeliveryListState } from './store'

export const getProductDeliveryApi = async (extraParams?: any) => {
  ...
  const data = getRecoil(productDeliveryListState)
  ...
  return axiosClient
    ...
    .then((res: IApiResponse | any) => {
      if (res.code == 200) {
        setRecoil(productDeliveryListState, res?.data?.productDeliveries?.data)
        setRecoil(totalPageState, res.data?.productDeliveries?.paginate?.total)
      }
    })
    .catch((err) => {
      setRecoil(productDeliveryListState, [])
...
export const submitCreateProductDeliveryApi = async (data: any, handleClose?: any) => {
  setRecoil(loadingState, true)

  return await axiosClient
    .post(endpoint.productDeliveries, data)
    .then((res: IApiResponse | any) => {
      if (res.code == 200) {
        toast.success(t('productDeliveryMessageCreateSuccess'))
...
export const submitUpdateProductDeliveryApi = async (
  data: any,
  productDeliveryId = 0,
  handleClose?: any
) => {
...
export const deleteProductDeliveryApi = async (productDeliveryId: number) => {
  setRecoil(loadingState, true)
  return axiosClient
    .delete(endpoint.productDeliveries + productDeliveryId)
    .then((res: IApiResponse | any) => {
      if (res.code == 200) {
        toast.success(t('productDeliveryMessageDeleteSuccess'))
        setRecoil(dataTableParamsState, (prevState) => {
          return {
            ...prevState,
            refetchData: prevState.refetchData + 1,
