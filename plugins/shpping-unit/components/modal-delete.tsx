...
import { deleteShippingUnitApi } from '../api'
...
export const ModalDelete = () => {
  ...
  const shippingUnitActive = useRecoilValue(shippingUnitActiveState)

  const handleDelete = () => {
    deleteShippingUnitApi(shippingUnitActive && shippingUnitActive?.id ? shippingUnitActive?.id : 0)
    ...
    <>
      ...
      >
        <Box textAlign={'center'}>
          ...
          <Typography>
            {t('shippingUnitMessageDeleteDescription')}{' '}
            <strong>"{shippingUnitActive?.name}"</strong>
          </Typography>

          <Stack direction={'row'} spacing={2} justifyContent="center" pt={2}>
            <ButtonBase
              btnText={t('shippingUnitBtnClose')}
              handleClick={() => {
                setOpenModalDelete(!openModalDelete)
              }}
            />
            <ButtonDelete btnText={t('shippingUnitBtnDelete')} handleClick={handleDelete} />
          </Stack>
        </Box>
      </MainModal>
    </>
