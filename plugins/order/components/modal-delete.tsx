...
export const ModalDelete = () => {
  ...
  const orderActive = useRecoilValue(orderActivedState)

  const handleDelete = () => {
    deleteOrderApi(orderActive && orderActive?.id ? orderActive?.id : 0)
    ...
    <>
      ...
      >
        <Box textAlign={'center'}>
          ...
          <Typography>
            {t('orderMessageDeleteDescription')} <strong>"#{padNumber(orderActive?.id)}"</strong>
          </Typography>

          <Stack direction={'row'} spacing={2} justifyContent="center" pt={2}>
            <ButtonBase
              btnText={t('orderBtnClose')}
              handleClick={() => {
                setOpenModalDelete(!openModalDelete)
              }}
            />
            <ButtonDelete btnText={t('orderBtnDelete')} handleClick={handleDelete} />
          </Stack>
        </Box>
      </MainModal>
    </>
