...
import { deleteInvestorApi } from '../api'
import { investorActiveState } from '../store'

export const ModalDelete = () => {
  const [openModalDelete, setOpenModalDelete] = useRecoilState(openModalDeleteState)
  const investorActive = useRecoilValue(investorActiveState)

  const handleDeleteRegion = () => {
    deleteInvestorApi(investorActive && investorActive?.id ? investorActive?.id : 0)
    ...
    <>
      ...
      >
        <Box textAlign={'center'}>
          ...
          <Typography>
            {t('investorMessageDeleteDescription')} <strong>"{investorActive?.name}"</strong>
          </Typography>

          <Stack direction={'row'} spacing={2} justifyContent="center" pt={4}>
            <ButtonBase
              btnText={t('investorBtnClose')}
              handleClick={() => {
                setOpenModalDelete(!openModalDelete)
              }}
            />
            <ButtonDelete btnText={t('investorBtnDelete')} handleClick={handleDeleteRegion} />
          </Stack>
        </Box>
      </MainModal>
    </>
