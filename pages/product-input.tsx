...
import { ProductInputContainer } from '@/plugins/product-input/components'
...
export default function ProductInputPage() {
  ...
  useEffect(() => {
    setRecoil(breadcrumbsState, [
      { name: t('dashboard'), to: '/' },
      { name: t('productInputBreadcrumbsTitle'), to: '' },
    ])
  ...
  useEffect(() => {
    if (accessToken) {
      if (!isUserLoading) {
        if (permissionsRule?.includes('product_input_view')) {
          setLoading(false)
          return
        } else {
          router.push('/404')
  ...
  return (
    <>
      <HeadPage title={t('productInputHeadTitle') ?? ''} />
      {loading ? <Loading open={true} /> : <ProductInputContainer />}
    </>
  )
}
